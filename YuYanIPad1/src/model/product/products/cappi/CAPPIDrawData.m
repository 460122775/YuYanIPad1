...
#define ProductHeadLength_CAPPI 4

@implementation CAPPIDrawData
    ...
    for (int x = xMin; x <= xMax; x++)
    {
        ...
        for (int y = yMin; y < yMax; y++)
        {
            ...
            if (iRb < self.iBinNumber)
            {
                ...
                ucharValue = (unsigned char*)[[data subdataWithRange:NSMakeRange(sizeof(self.fileHeadStruct) + ProductHeadLength_CAPPI + self.sizeofRadial * seta + RadialHeadLength + iRb * sizeof(unsigned char), sizeof(unsigned char))] bytes];
                value = ucharValue[0];
                if (value > 1)
                {
