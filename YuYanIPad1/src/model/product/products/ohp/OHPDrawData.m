...
@implementation OHPDrawData
@synthesize OHPHeadStruct;
...
{
    [data getBytes:&(self->OHPHeadStruct) range:NSMakeRange(sizeof(self.fileHeadStruct), sizeof(self.OHPHeadStruct))];
    self.maxWindowCount = self.OHPHeadStruct.maxLayCell;
    self.windowSize = self.OHPHeadStruct.LenOfWin;
    ...
    for (int x = xMin; x <= xMax; x++)
    {
        ...
        for (int y = yMin; y < yMax; y++)
        {
            ...
            if (s < self.maxRadarDistance * 1000)
            {
                ...
                floatValue = (float*)[[data subdataWithRange:NSMakeRange(sizeof(self.fileHeadStruct) + sizeof(self.OHPHeadStruct) + ((cellNo - 1) * self.maxWindowCount * self.maxWindowCount + abs(yPosition) * self.maxWindowCount + abs(xPosition)) * sizeof(float),
                                                   sizeof(float))] bytes];
                value = floatValue[0];
                if (value > 0)
                {
