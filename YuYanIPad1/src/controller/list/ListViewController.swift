...
class ListViewController : UIViewController, ListLeftViewProtocol, SwitchToolDelegate, ProductViewADelegate, UIImagePickerControllerDelegate, CartoonBarDelegate
{
    ...
    override func viewDidLoad()
    {
        ...
        // Product Left view.
        self.listLeftView = (NSBundle.mainBundle().loadNibNamed("ListLeftView", owner: self, options: nil) as NSArray).lastObject as? ListLeftView
        self.listLeftView?.frame.origin = CGPointMake(-240, 0)
        self.listLeftView?.listLeftViewDelegate = self
        self.view.addSubview(self.listLeftView!)
        // Init product view.
        ...
        NSNotificationCenter.defaultCenter().addObserver(self, selector:
            #selector(ListViewController.receiveProductFromSocket(_:)), name: "\(RECEIVE)\(SOCKET)\(PRODUCT)", object: nil)
        // Init user location.
    ...
    override func viewWillAppear(animated: Bool)
    {
        ...
        NSNotificationCenter.defaultCenter().addObserver(
            self, selector: #selector(ListViewController.receiveDataFromHttp(_:)),
            name: "\(PRODUCT)\(HTTP)\(SELECT)\(SUCCESS)", object: nil)
        NSNotificationCenter.defaultCenter().addObserver(
            self, selector: #selector(ListViewController.receiveRadarStatus(_:)),
            name: "\(RECEIVE)\(RADARSTATUS)", object: nil)
        NSNotificationCenter.defaultCenter().addObserver(
            self, selector: #selector(ListViewController.appActiveControl(_:)),
            name: "\(APP_ACTIVE)", object: nil)
        //Use for cartoon.
        NSNotificationCenter.defaultCenter().addObserver(
            self, selector: #selector(ListViewController.receiveCartoonDataFromHttp(_:)),
            name: "\(CARTOON)\(HTTP)\(SELECT)\(SUCCESS)", object: nil)
        // Init radar status.
    ...
    func receiveProductFromSocket(notificaiton : NSNotification)
    {
        ...
        self.listLeftView!.refreshListTable()
    }
    ...
    func drawProduct(data : NSData)
    {
        ...
        self.listLeftView?.setProductLeftViewByData(data)
    ...
    func initProductInfoByData()
    {
        ...
        {
            self.listLeftView?.setProductLeftViewByData(currentProductData!)
        }
    }
